...
<xsl:template match="remark|convention|note|observation|warning|insight|computation|technology|example|question|problem|project|activity|exploration|investigation|definition|theorem|corollary|lemma|algorithm|proposition|claim|fact|identity|axiom|conjecture|principle|heuristic|hypothesis|assumption" mode="slides">
  <xsl:call-template name="start-slide"/>
  <xsl:apply-templates select="."/>
  <xsl:call-template name="end-slide"/>
</xsl:template>
