...
# for the two PROJECT repositories
SRC = $(PROJECT)/ptx
XSL = $(PROJECT)/xsl

# These are the files to apply templates to
MAIN = $(SRC)/aata.ptx
MAINNOTES = $(SRC)/math322-notes.ptx
...
ws-tex:
	install -d $(WORKSHEETS)
	cp -a $(SRC)/images $(WORKSHEETS)
	cd $(WORKSHEETS); \
	xsltproc --xinclude $(XSL)/pretext-latex-extras.xsl $(MAIN)

worksheets: ws-tex
	cd $(WORKSHEETS); \
	$(ENGINE) worksheets.tex
	cd $(WORKSHEETS); \
	for f in ws_*.tex; do $(ENGINE) $$f; done

ws-new:
	install -d $(WORKSHEETS)
	cp -a $(SRC)/images $(WORKSHEETS)
	cd $(WORKSHEETS); \
	xsltproc --xinclude $(XSL)/pretext-latex-extras.xsl $(SRC)/activity-new.ptx; \
	$(ENGINE) activity-new.tex; \
	$(PDFVIEWER) activity-new.pdf &
...
html:
	...
	cd $(HTMLOUT); \
	xsltproc --xinclude --stringparam publisher ../publisher/public.xml $(XSL)/custom-html.xsl $(MAIN);
...
latex:
	...
	cp -a $(SRC)/images $(PDFOUT)
	cd $(PDFOUT); \
	xsltproc --xinclude $(XSL)/custom-latex.xsl $(MAINNOTES) > main.tex;

# latex-fresh: ww-fresh latex
	
pdf:
	cd $(PDFOUT); \
	$(ENGINE) main.tex; $(ENGINE) main.tex;


notes: latex pdf
	cp $(PDFOUT)/main.pdf notes.pdf

all: html notes worksheets

all-diagrams: diagrams all
