...
\firstpageheader{MATH 322}{\bf\large Exam 1 -- Take Home\\Solutions}{Spring 2020}
...
\begin{solution}
Consider $F = \Q(\alpha+\alpha^6)$.  This is certainly a subfield of $E$ and has the property that complex conjugation fixes every element (since $\alpha$ and $\alpha^6$ are complex conjugates).  Thus fixer of $F$ is $\{\sigma_1, \sigma_6\}$. 
\end{solution}
\vfill
...
\begin{solution}
 Let $H = \{1, 2, 4\}$, or in terms of automorphisms, $\{\sigma_1, \sigma_2, \sigma_4\}$.  What elements of $E$ do these fix?  Well, $\sigma_2(\alpha) = \alpha^2$ and $\sigma_2(\alpha^2) = \alpha^4$ and $\sigma_2(\alpha^4) = \alpha$.  So if we consider the element $\alpha + \alpha^2 +\alpha^4$ we notice that $\sigma_2$ fixes it.  It is easy to see that $\sigma_4$ also fixes it.  Thus the fixfield of $H$ will be $F' = \Q(\alpha+\alpha^2+\alpha^4)$.  

 We can say a little more.  Since the index of $H$ in $G$ is $[G:H] = 6/3 = 2$, we know that the degree of $F'$ over $\Q$ will be $[F':\Q] = 2$.  This in turn tell us that the number $\alpha + \alpha^2+\alpha^4$ is the root of a degree 2 polynomial.
\end{solution}
\vfill
\end{parts}
